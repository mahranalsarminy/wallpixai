@php
    $google_analytics = extension('google_analytics');
    $tawk_to = extension('tawk_to');
    $popup = settings('popup');
@endphp
@if ($google_analytics->status)
    <script async src="https://www.googletagmanager.com/gtag/js?id={{ $google_analytics->credentials->measurement_id }}">
    </script>
    <script>
        ...
        gtag("js", new Date());
        gtag("config", "{{ $google_analytics->credentials->measurement_id }}");
    </script>
@endif
@if ($tawk_to->status)
    <script type='text/javascript'>
        ...
        (function() {
            ...
            s1.async = true;
            s1.src = 'https://embed.tawk.to/{{ $tawk_to->credentials->api_key }}';
            s1.charset = 'UTF-8';
...
@endif
@if ($popup->status && !request()->hasCookie('popup_closed'))
...
@if (settings('actions')->gdpr_cookie_status && !request()->hasCookie('cookie_accepted'))
    <div class="cookies">
        <div class="cookies-img"> <svg id="Capa_1" enable-background="new 0 0 80 80" height="80"
                viewBox="0 0 512 512" width="80" fill="{{ settings('colors')->primary_color }}"
        ...
        <div class="d-flex justify-content-center">
            ...
            @if (settings('general')->terms_of_service_link)
                <a class="btn btn-outline-primary px-5"
                    href="{{ settings('general')->terms_of_service_link }}">{{ lang('More') }}</a>
            @endif
        </div>
    </div>
@endif
