...
function hashid($id, $length = 12)
{
    $hashids = new Hashids('', $length);
    return $hashids->encode($id);
}

function unhashid($id, $length = 12)
{
    $hashids = new Hashids('', $length);
...
function countryTax($countryName)
{
    ...
    if (is_null($country)) {
        $country = Country::where('code', ipInfo()->location->country_code)->first();
    }
    if (!is_null($country)) {
...
function engine($alias)
{
    return Engine::where('alias', $alias)
        ->active()->first();
}
